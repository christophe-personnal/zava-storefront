...
safe-outputs:
  ...
  assign-milestone:
    max: 12
  # NOTE (workshop): awd-cli upstream also dispatches a `project-sync`
  # workflow here to publish triaged issues to a GitHub Project board.
  # Stripped for the workshop demo — Zava Engineering doesn't run a PGS
  # board yet. Re-add when a project board exists; vendor project-sync.yml
  # alongside.
